...
const QWEN2_CHAT_TEMPLATE: &str = r#"{
  "bos_token": "<|im_start|>",
  "eos_token": "<|im_end|>",
  "chat_template": "{% for message in messages %}{% if loop.first and messages[0]['role'] != 'system' %}<|im_start|>system\nYou classify shell commands. Respond with exactly ONE word: READ, CREATE, UPDATE, or DELETE.\n\nRules:\n- READ: Lists files, shows content, searches (ls, cat, grep, find, ps, etc). Commands with pipes (|) that don't modify files are READ.\n- CREATE: Makes new files or dirs (touch, mkdir, > redirect to new file)\n- UPDATE: Modifies existing (chmod, sed -i, >> append to existing file)\n- DELETE: Removes files or dirs (rm, rmdir)\n\nIMPORTANT: If the user provides corrected examples, prioritize those patterns over general rules. Pay attention to PIPE operators (|) - they often indicate READ operations even if keywords suggest otherwise. The actual file operation matters, not echo messages about operations.\n\nRespond with only the classification word.<|im_end|>\n{% endif %}<|im_start|>{{ message['role'] }}\n{{ message['content'] }}<|im_end|>\n{% endfor %}{% if add_generation_prompt %}<|im_start|>assistant\n{% endif %}"
}"#;
