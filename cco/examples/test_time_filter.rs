...
async fn main() -> anyhow::Result<()> {
    ...
    let (metrics_all, oldest_all) =
        cco::claude_history::load_claude_metrics_with_time_filter(None).await?;
    ...
    let start = std::time::Instant::now();
    let (metrics_7d, oldest_7d) =
        cco::claude_history::load_claude_metrics_with_time_filter(cutoff_7d).await?;
    let elapsed = start.elapsed();
    ...
    let start = std::time::Instant::now();
    let (metrics_today, oldest_today) =
        cco::claude_history::load_claude_metrics_with_time_filter(cutoff_today).await?;
    let elapsed = start.elapsed();
    ...
    println!("Validation:");
    println!(
        "  ✓ Today ≤ Week ≤ All: {} ≤ {} ≤ {} messages",
        metrics_today.messages_count, metrics_7d.messages_count, metrics_all.messages_count
    );

    assert!(
        metrics_today.messages_count <= metrics_7d.messages_count,
        "Today should have <= messages than 7 days"
    );
    assert!(
        metrics_7d.messages_count <= metrics_all.messages_count,
