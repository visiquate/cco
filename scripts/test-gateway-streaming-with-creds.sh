...
# LLM Gateway Streaming Test Script (with credential system)
...
# Step 2: Get API key from environment or credential system
...
if [ -n "${ANTHROPIC_API_KEY:-}" ]; then
    ...
    # Try credential system
    if command -v cco &>/dev/null; then
        API_KEY=$(cco credentials retrieve ANTHROPIC_API_KEY 2>/dev/null || echo "")
        if [ -n "$API_KEY" ]; then
            echo -e "${GREEN}✓ Using API key from credential system${NC}"
        else
            echo -e "${RED}✗ Could not retrieve API key${NC}"
            echo "Please set ANTHROPIC_API_KEY environment variable or store in credential system:"
            echo "  export ANTHROPIC_API_KEY=your-key"
            echo "  OR"
            echo "  cco credentials store ANTHROPIC_API_KEY your-key"
            exit 1
        fi
    else
        echo -e "${RED}✗ cco command not found and ANTHROPIC_API_KEY not set${NC}"
        exit 1
    fi
fi
echo ""
...
echo "  - Monitor audit logs with: cco daemon logs"
