...
2. **Orchestra Config** (`/Users/brent/git/cc-orchestra/config/orchestra-meta.json`)
   - 117 agent definitions (1 Opus, 35 Sonnet, 81 Haiku)
...
The orchestra automatically selects the right agents from 117 available based on:

1. **Technology Keywords**
   - "Python" → Python Expert (Sonnet)
   - "Flutter" → Flutter Expert (Sonnet)
   - "Go" → Go Expert (Sonnet)
   - "Rust" → Rust Expert (Sonnet)
   - "Swift"/"iOS" → Swift Expert (Sonnet)

2. **Integration Keywords**
   - "Salesforce" → Salesforce API Expert (Sonnet)
   - "Authentik"/"OAuth2"/"SAML" → Authentik API Expert (Sonnet)
   - "API integration" → API Explorer (Sonnet)

3. **Infrastructure Keywords**
   - "Docker"/"Kubernetes"/"AWS" → DevOps Engineer (Sonnet)
   - "Deploy"/"CI/CD" → DevOps Engineer (Sonnet)

4. **Quality Keywords**
   - "tests"/"testing" → QA Engineer (Sonnet)
   - "security"/"audit" → Security Auditor (Sonnet)
   - "documentation"/"docs" → Documentation Expert (Haiku)

5. **Always Included**
   - Chief Architect (Opus) - coordinates everything
   - Security Auditor (Sonnet) - security is critical
   - QA Engineer (Sonnet) - quality is critical
...
- **Orchestra Config**: `/Users/brent/git/cc-orchestra/config/orchestra-meta.json`
