...
CCO includes a built-in auto-update system that checks for new versions and notifies you when updates are available. Updates are released regularly using a sequential versioning (YYYY.MM.N) scheme (YYYY.MM.N format), making it easy to identify when each version was released.
