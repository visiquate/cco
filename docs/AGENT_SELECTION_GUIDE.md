...
This guide helps you select the right agents for your project from the Claude Orchestra's **117 specialized agents**. Smart agent selection is key to efficient, high-quality development.

> **Note:** Agent model assignments (Opus / Sonnet / Haiku) reflect the **default ("balanced") mapping** and are remappable via `agent_tier` / `[model_tiers]` in your config — see [configuration.md](./configuration.md#model-tiers).
...
- **Config File**: `/Users/brent/git/cc-orchestra/config/orchestra-meta.json`
- **Usage Guide**: [ORCHESTRA_USAGE_GUIDE.md](ORCHESTRA_USAGE_GUIDE.md)

---

**Smart Selection = Efficient Development** | **117 Agents** | **Right Tool for Every Job**
